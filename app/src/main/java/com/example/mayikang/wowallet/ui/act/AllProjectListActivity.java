 ...
 * 项目列表页
 */
@Route(path = "/main/act/all_project_list")
public class AllProjectListActivity extends BaseAppcompatActivity {
    ...
    //查询店铺项目
    public void pullStoreProject() {
        pageIndex = 1;
        data.clear();
        HashMap<String, String> map = new HashMap<>();
        map.put("ctype", "goods");
        map.put("pageIndex", String.valueOf(pageIndex));
        map.put("cond", "{state:2,isDelete:1,isCertify:2,goodsType:{isDelete:1}}");
    ...
    public void pullMoreStoreProject() {
        HashMap<String, String> map = new HashMap<>();
        map.put("ctype", "goods");
        map.put("cond", "{state:2,isDelete:1,isCertify:2,goodsType:{isDelete:1}}");
        map.put("jf", "goodsType|photo");
        ...
        http.doCommonPost(null, MainUrl.basePageQueryUrl, map, new XProgressCallback() {
            @Override
            public void onSuccess(String resultStr) {
                Log.e("MoreStoreProject", resultStr.toString());
                if (!StringUtil.isBlank(resultStr)) {
                    try {
                        JSONObject obj = new JSONObject(resultStr);
