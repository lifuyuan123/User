...
public class HttpServiceImpl implements IHttpService {
    ...
    public void doCommonPost(@Nullable Map<String,String> headers, @NonNull String url, Map<String, String> pParameters, final XProgressCallback callback) {
        ...
        try {
            ...
            Log.e("TOKEN",SPFUtil.get(Tag.TAG_TOKEN,"default").toString()+"-------DEV-ID:"+"ANDROID_"+DeviceUtil.getDeviceId(config.getContext()));
        ...
        if(pParameters!=null && pParameters.size()>0){
          ...
          LogUtil.e("location",requestParams.toString()+"header"+requestParams.getHeaders().toString());
            ...
            public void onSuccess(NormalResponse result) {
                ...
                if(null!=cookies&&cookies.size()>0){
                    for (int i = 0; i <cookies.size() ; i++) {
                        HttpCookie currCookie=cookies.get(i);
                        if(currCookie.getName().equals("JSESSIONID")){
                           SPFUtil.put("JSESSIONID",currCookie.getValue());
                            Log.e("currCookie",currCookie.getValue());
                        }
                    }
                }
